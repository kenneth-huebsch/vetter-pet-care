...
class Charge_SettingsController extends Charge_BaseCpController
{
    ...
    public function actionCharges(array $variables = [])
    {
        $variables = array_merge($variables, [
            ...
            'urlFormat' => craft()->charge_charge->getUrlFormat(),
            'elementTemplate' => craft()->charge_charge->getElementTemplate()
        ]);
    ...
    public function actionSaveChargeSettings(array $variables = [])
    {
        ...
        if($urlFormat == '') {
            ...
            if(strpos($urlFormat, '{hash}') === false) {
                $errors['urlFormat'] = 'Url format must contain {hash} within the pattern';
            }
        }
        ...
        if(!empty($errors)) {
            ...
            return $this->actionCharges($variables);
    ...
    public function actionConnect(array $variables = [])
    {
        $variables = [
            ...
            'isPro'             => craft()->charge_license->isProEdition()
        ];
