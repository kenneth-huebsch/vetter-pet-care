...
*/
        ...
        switch ($attribute) {
            ...
            case 'amount':  {
                if ($element->type == 'recurring') {
                    return $element->formatPlanName('symbol');
                } else {
            ...
            */
            ...
            case 'accountId': {
                ...
                $url = UrlHelper::getCpUrl('charge/connect/'.$account->id);

                return '<a href="'.$url.'">'.$account->title().'</a>';
            }
    ...
    {
        $query
            ->addSelect('charges.userId, charges.sourceUrl, charges.type, charges.customerId, charges.accountId, charges.mode, charges.description, charges.timestamp, charges.hash, charges.notes,
        ...
        if ($criteria->accountId) {
            $query->andWhere(DbHelper::parseParam('charges.accountId', $criteria->accountId, $query->params));
        }
