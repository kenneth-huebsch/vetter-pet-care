...
class Charge_ChargeService extends BaseApplicationComponent
{
    ...
    private function handlePaymentOneoff(Charge_CustomerModel $customer, $paymentCardId, $model)
    {
        ...
        try {
            ...
            $model = craft()->charge_connect->setupModelForConnect($model);

            if($model->destination != null && $model->applicationFeeInCents != '' && $model->applicationFeeInCents > 0) {

                if(craft()->charge_connect->getConnectBehaviour() == 'direct') {
                    $stripe::accountId($model->destination);
                } else {
                    $chargeAttr['destination'] = $model->destination;
                }

                if($model->applicationFeeInCents != '' && $model->applicationFeeInCents > 0) {
                    $chargeAttr['application_fee'] = $model->applicationFeeInCents;
                }
            }
            ...
            $response = $stripe->charges()->create($chargeAttr);
            $this->record('one-off', $model, ['customer' => $customer, 'payment' => $response]);

            return $response;

        } catch (\Exception $e) {
            craft()->charge_log->exception('Failed during charge creation with message : ' . $e->getMessage());
            $model->addError('general', 'Failed during charge creation with message : ' . $e->getMessage());
    ...
    public function record($type = 'one-off', ChargeModel $model, $extra = array())
    {
        ...
        if (craft()->elements->saveElement($model, false)) {
            ...
            $record->currency = $model->planCurrency;
            $record->amount = $model->planAmount;
            $record->insert();
    ...
    private function setRequestForCharge(ChargeModel $model)
    {
        ...
        $request['applicationFee'] = $model->applicationFee;
        $request['applicationFeeInPercent'] = $model->applicationFeeInPercent;
        $request['applicationFeeInCents'] = $model->applicationFeeInCents;
