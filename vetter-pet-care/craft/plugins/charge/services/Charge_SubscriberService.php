...
class Charge_SubscriberService extends BaseApplicationComponent
{
	...
	/**
	 * (Contributed by Renoît Rouleau)
	 *
	 * @param $newMembershipSubscriptionName
	 * @param ChargeModel $charge
	 * @return bool
	 * @throws \Exception
	 */
	public function changeSubscription($newMembershipSubscriptionName, ChargeModel $charge)
	{
		craft()->charge_log->action('Changing Subscription for User', ['newMembershipSubscriptionName' => $newMembershipSubscriptionName]);

		$subscriber = craft()->charge_subscriber->getSubscriberByChargeId($charge->id);
		if (is_null($subscriber)) {
			craft()->charge_log->error('No matching subscriber for charge', ['charge' => $charge]);
			return false;
		}

		$user = $subscriber->user();
		if (is_null($user)) {
			craft()->charge_log->error('No matching user for subscriber', ['subscriber' => $subscriber]);
			return false;
		}

		$oldMembershipSubscription = craft()->charge_membershipSubscription->getMembershipSubscriptionById($subscriber->membershipSubscriptionId);
		if (is_null($oldMembershipSubscription)) {
			craft()->charge_log->error('Failed to find the old membership subscription for subscriber', ['subscriber' => $subscriber]);
			return false;
		}

		$newMembershipSubscription = craft()->charge_membershipSubscription->getMembershipSubscriptionByHandle($newMembershipSubscriptionName);
		if (is_null($newMembershipSubscription)) {
			craft()->charge_log->error('Failed to find a membership subscription with the handle "' . $newMembershipSubscriptionName . '"');
			return false;
		}

		$subscriber->membershipSubscriptionId = $newMembershipSubscription->id;

		if (craft()->elements->saveElement($subscriber, false)) {
			$subscriberRecord = Charge_SubscriberRecord::model()->findById($subscriber->id);
			$subscriberRecord->membershipSubscriptionId = $newMembershipSubscription->id;
		...
		$groupIds = [];
		foreach ($user->getGroups() as $group) {
			if ($group->id !== $oldMembershipSubscription->activeUserGroup) {
				$groupIds[] = $group->id;
			}
		}
		if (!in_array($newMembershipSubscription->activeUserGroup, $groupIds)) {
			$groupIds[] = $newMembershipSubscription->activeUserGroup;
		}

		craft()->userGroups->assignUserToGroups($user->id, $groupIds);
		craft()->charge_log->action('Subscription modified for "' . $user->email . '". User has been removed from the active user group ' . $oldMembershipSubscription->activeUserGroup . ' and added to the active user group ' . $newMembershipSubscription->activeUserGroup);
	}
    ...
    {
        craft()->charge_log->note('Finding subscriber details to trigger recurring subscriber action', ['charge' => $chargeModel]);
