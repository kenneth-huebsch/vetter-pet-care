...
class ChargeService extends BaseApplicationComponent
{
    ...
    public function handleCustomerCharge(){
    echo craft()->request->getPost('amnt');
    // return $obj;
  /*     $this->requirePostRequest();
        ...
        if($valid){
            craft()->charge->handlePayment($this->charge);
        } else {
            return "error";
        }
    ...
    */
    ...
    $customerModel = craft()->charge_customer->find();
        return $customerModel->getCard();
        //print_r($customerModel->getCard());
    }
   ...
   public function handleGetOrCreate(ChargeModel &$model, $chargeModifiers = [])
    {
      /*  $event = new Event($this, ['charge' => $model]);
        $this->onBeforeCharge($event);
        if($event->performAction === false) {
