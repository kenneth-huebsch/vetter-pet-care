...
class ChargeModel extends BaseElementModel
{
    ...
    public function applicationFeeFormatted()
    {
        return $this->_formatAmount(round($this->applicationFeeInCents / 100), $this->planCurrency, 'symbol');
    }
    ...
    public function validate($attributes = null, $clear = true)
    {
        ...
        // Setup the application fee in cents value
        if (is_numeric($this->applicationFee)) {
            $this->applicationFeeInCents = floor($this->applicationFee * 100);
        } elseif( is_numeric($this->applicationFeeInPercent)) {
            $this->applicationFeeInCents = floor($this->planAmountInCents * ( $this->applicationFeeInPercent / 100 ));
        }
    ...
    {
        return craft()->charge_connect->getAccountById($this->accountId);
    }
    ...
    protected function defineAttributes()
    {
        return array_merge(parent::defineAttributes(), [
            ...
            'accountId'             => [AttributeType::Number],
            'destination'           => [AttributeType::String],
            'applicationFee'        => [AttributeType::Number, 'decimals' => 2],
            'applicationFeeInCents' => [AttributeType::Number],
            'applicationFeeInPercent' => [AttributeType::Number]
        ]);
    }
