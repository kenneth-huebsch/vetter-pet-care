...
class TwilioSmsController extends BaseController {
  ...
  public function actionSendSms() {
    ...
    // Modify to get customer cell phone
      $_POST['sms_user_phone'] = craft()->request->getRequiredPost('TextTo');
 ...
 if (craft()->request->getRequiredPost('textType')  == 'custApptReminderTxt'){
     $message = $settings['custApptReminderTxt'];
 } elseif (craft()->request->getRequiredPost('textType')  == 'custApptCancelledTxt'){
     $message = $settings['custApptCancelledTxt'];
 } elseif (craft()->request->getRequiredPost('textType')  == 'vet24hrsApptNotificationTxt'){
    ...
    // no errors, send the sms
      ...
      // 08-07-2017 achan need to use dynamic "to" phone number
        if (craft()->request->getRequiredPost('TextTo') == ''){
            $toArr = explode("\r\n", $settings['to']);
            TwilioSmsPlugin::log($toArr[0],LogLevel::Info,true);
        }else{
            $toArr = explode("\r\n", craft()->request->getRequiredPost('TextTo'));
            $logmessage = "Send SMS message to ".$toArr[0].' '. craft()->request->getRequiredPost('textType').' from controller.';
