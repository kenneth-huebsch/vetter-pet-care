...
class AmForms_SubmissionsController extends BaseController
{
    ...
    public function actionSaveSubmission()
    {
        ...
        // Zipcode entered from form
        $entered_zipcode = craft()->request->getPost($namespace.'.fields.requestZipcode');

        // Query to lookup zipcode
        $zipcode_lookup_query = craft()->db->createCommand()
            ->select('id, data')
            ->from('entryversions')
            ->where(array('and', 'sectionId = 4', 'data regexp \'"title":"'.$entered_zipcode.'"\''))
            ->queryRow()
        ;

        /*
         * Response
         * Array[id] => 175, [field_requestZipcode] => 19030)
         * */
        if( $zipcode_lookup_query['id'] ) {
            // Return the submission by the form's handle, for custom HTML possibilities
            craft()->urlManager->setRouteVariables(array("existingZipcodeFound" => TRUE));

            // Need to return; otherwise, the form will submit
            return;
        }
