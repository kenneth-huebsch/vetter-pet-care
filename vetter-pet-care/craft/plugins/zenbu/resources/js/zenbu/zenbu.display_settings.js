...
function saveSearch()
{
    ...
    var theData = $('form#saveSearch').serialize();
    ...
    $.ajax({
        ...
        success: function(results){
            $.each(results, function(i, result) {
                ...
                if(! _.includes(['sectionId', 'entryTypeId', 'limit', 'orderby'], result.filterAttribute1))
                {
        ...
        stop: function(event, ui) {
            if($(this).closest('table').hasClass('displaySettings'))
            {
                $(this).find('tr').each(function(i) {
                    ...
                    $(this).find('td').eq(4).find('input, select, textarea').each(function(i2) {
                        newName = $(this).attr('name').replace(/\[([0-9].*?)\]\[(.*?)\]/g, '[' + i + '][$2]');
                        $(this).attr('name', newName);
                    });
                });
...
$(document).ready(function ()
{
    ...
    //    ----------------------------------------
    //    Display Settings
    //    Custom Display Template Textarea toggle
    //    ----------------------------------------
    $('body').delegate('button.toggleCustomDisplayTemplateField', 'click', function () {
        if($(this).siblings('div.customDisplayTemplateFieldWrapper').is(':hidden'))
        {
            $(this).siblings('div.customDisplayTemplateFieldWrapper').slideDown(200);
            $(this).siblings('code.sample').fadeOut(200);
            $(this).find('i.fa').removeClass('fa-angle-right').addClass('fa-angle-down');
        }
        else
        {
            $(this).siblings('div.customDisplayTemplateFieldWrapper').slideUp(200);
            codeVal = $(this).siblings('div.customDisplayTemplateFieldWrapper').find('textarea').val();
            truncateLength = 30;
            codeVal =  codeVal.length > codeVal.substring(0,truncateLength).length ? codeVal.substring(0,truncateLength) + '...' : codeVal.substring(0,truncateLength);
            $(this).siblings('code.sample').text(codeVal).fadeIn(200).html();
            $(this).find('i.fa').removeClass('fa-angle-down').addClass('fa-angle-right');
        }
    });

    $('body').delegate('textarea.customDisplayTemplateField', 'keyup', function () {
        var contentLength = $(this).val().length;
        if(contentLength > 0)
        {
            $(this).closest('div.customDisplayTemplateFieldWrapper').siblings('button').find('.containsData').hide();
        }
        else
        {
            $(this).closest('div.customDisplayTemplateFieldWrapper').siblings('button').find('.containsData').show();
        }
    });
