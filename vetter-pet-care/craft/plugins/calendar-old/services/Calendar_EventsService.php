    ...
    public function saveEvent(Calendar_EventModel $event, $validateContent = null)
    {
        ...
        if (!$isNewEvent) {
            ...
            $result = craft()->db
                ...
                ->join('elements elements', 'elements_i18n.elementId = elements.id')
                ->where('elements_i18n.slug = :slug', array('slug' => $event->slug))
                ->andWhere('elements_i18n.elementId != :elementId', array('elementId' => $event->id))
                ->andWhere('elements.type != :elementType', array('elementType' => ElementType::Asset))
                ->query();
            ...
            if ($result->getRowCount() != 0) {
                $event->addError('slug', Craft::t('Slug already exists'));
            }
        }

        if (!$event->getCalendar()->hasTitleField) {
            $event->getContent()->title = craft()
                ->templates
